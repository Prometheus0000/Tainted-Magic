...
/**
 * Copy of <Mojang> WorldGenBigTree.class
 */
public class WorldGenWarpwoodTree extends WorldGenAbstractTree
{
     ...
     */
    static final byte[] otherCoordPairs = new byte[]{
            (byte) 2,
            (byte) 0,
            (byte) 0,
            (byte) 1,
                ...
                for (double d0 = 0.5D; j1 < i; ++j1)
                {
                    ...
                    if (this.checkBlockLine(aint1, aint2) == -1)
                    {
                        int[] aint3 = new int[]{
                                ...
                                this.basePos[2]};
                        double d3 =
                                Math.sqrt(Math.pow((double) Math.abs(this.basePos[0] - aint1[0]), 2.0D) + Math.pow((double) Math.abs(this.basePos[2] - aint1[2]), 2.0D));
        ...
        for (aint1[p_150529_5_] = aint[p_150529_5_]; i1 <= l; ++i1)
        {
            ...
            while (j1 <= l)
            {
                ...
                else
                {
                    ...
                    if (!block1.isAir(worldObj, aint1[0], aint1[1], aint1[2]) && !block1.isLeaves(worldObj, aint1[0],
                            aint1[1], aint1[2]))
                    {
                        ++j1;
                    }
    ...
    float leafSize(int p_76495_1_)
    {
        return p_76495_1_ >= 0 && p_76495_1_ < this.leafDistanceLimit ?
                (p_76495_1_ != 0 && p_76495_1_ != this.leafDistanceLimit - 1 ? 3.0F : 2.0F) : -1.0F;
    }
    ...
    void generateTrunk()
    {
        ...
        int[] aint1 = new int[]{
                ...
                k,
                l};
        this.func_150530_a(aint, aint1, trunk);
    ...
    void generateLeafNodeBases()
    {
                ...
                this.basePos[2]}; i < j; ++i)
        {
            ...
            {
                this.func_150530_a(aint, aint2, trunk);
            }
        }
    }
    ...
    boolean validTreeLocation()
    {
        ...
        boolean isSoil = block.canSustainPlant(worldObj, basePos[0], basePos[1] - 1, basePos[2], ForgeDirection.UP, (BlockSapling) BlockRegistry.BlockWarpwoodSapling);
        if (!isSoil)
        {
            return false;
        }
