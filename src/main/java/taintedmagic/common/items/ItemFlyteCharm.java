...
public class ItemFlyteCharm extends Item implements IWarpingGear, IRenderInventoryItem
{
        ...
        if (event.entityLiving instanceof EntityPlayer)
        {
            ...
            if (s != null)
            {
                ...
                if (isFlying)
                {
                    ...
                    {
                        p.worldObj.playSound(p.posX, p.posY, p.posZ, "taintedmagic:burst", 5.0F,
                                1.0F + (float) Math.random() * 0.1F, true);
    ...
    private boolean shouldPlayerHaveFlight(EntityPlayer p)
    {
        for (int i = 0; i < p.inventory.getSizeInventory(); i++)
            if (p.inventory.getStackInSlot(i) != null && p.inventory.getStackInSlot(i).getItem() instanceof ItemFlyteCharm && TaintedMagicHelper.consumeVisFromInventory(p, cost, false))
                return true;
        return false;
    }
