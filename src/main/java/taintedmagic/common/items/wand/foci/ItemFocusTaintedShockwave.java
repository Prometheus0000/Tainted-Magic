...
public class ItemFocusTaintedShockwave extends ItemFocusBasic
{
    private static final AspectList cost =
            new AspectList().add(Aspect.ENTROPY, 1000).add(Aspect.EARTH, 1000).add(Aspect.WATER, 1000).add(Aspect.ORDER, 500);
    ...
    public void addInformation(ItemStack s, EntityPlayer p, List l, boolean b)
    {
        ...
        l.add(EnumChatFormatting.BLUE + "+" + new String(this.isUpgradedWith(s, FocusUpgradeType.enlarge) ?
                Integer.toString(15 + this.getUpgradeLevel(s, FocusUpgradeType.enlarge)) : "15") + " " + StatCollector.translateToLocal("text.radius"));
    }
    ...
    public ItemStack onFocusRightClick(ItemStack s, World w, EntityPlayer p, MovingObjectPosition mop)
    {
        ...
        {
            List<EntityLivingBase> ents = w.getEntitiesWithinAABB(EntityLivingBase.class,
                    AxisAlignedBB.getBoundingBox(p.posX, p.posY, p.posZ, p.posX + 1, p.posY + 1, p.posZ + 1).expand(15.0D + this.getUpgradeLevel(s, FocusUpgradeType.enlarge), 15.0D + this.getUpgradeLevel(s, FocusUpgradeType.enlarge), 15.0D + this.getUpgradeLevel(s, FocusUpgradeType.enlarge)));
    ...
    public void spawnParticles(World w, EntityPlayer p)
    {
        for (int i = 1; i < 360; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                ...
                FXWisp ef = new FXWisp(w, p.posX + xp + off, p.posY - 1, p.posZ + zp + off,
