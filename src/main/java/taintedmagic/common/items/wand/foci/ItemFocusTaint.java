...
public class ItemFocusTaint extends ItemFocusBasic
{
    ...
    private static final AspectList costAntibody =
            new AspectList().add(Aspect.EARTH, 10).add(Aspect.WATER, 10).add(Aspect.ORDER, 10);
    ...
    public void onUsingFocusTick(ItemStack s, EntityPlayer p, int count)
    {
        ...
        if (!p.worldObj.isRemote && p.ticksExisted % 5 == 0)
            p.worldObj.playSoundAtEntity(p, "thaumcraft:bubble", 0.33F, 5.0F * (float) Math.random());

        if (!p.worldObj.isRemote && wand.consumeAllVis(s, p, getVisCost(s), true, false))
        {
            for (int a = 0; a < 2 + wand.getFocusPotency(s); a++)
            {
                EntityTaintBubble proj = new EntityTaintBubble(p.worldObj, p, isUpgradedWith(wand.getFocusItem(s),
                        FocusUpgradeType.enlarge) ? 15.0F : 8.0F, 4F + wand.getFocusPotency(s),
