...
public class ItemFocusDarkMatter extends ItemFocusBasic
{
    ...
    private static final AspectList costSanity =
            new AspectList().add(Aspect.ENTROPY, 20).add(Aspect.FIRE, 50).add(Aspect.ORDER, 50);
    private static final AspectList costCorrosive =
            new AspectList().add(Aspect.ENTROPY, 20).add(Aspect.FIRE, 50).add(Aspect.WATER, 20);
    ...
    {
        return isUpgradedWith(s, FocusUpgrades.corrosive) ? costCorrosive : isUpgradedWith(s, FocusUpgrades.sanity) ?
                costSanity : cost;
    }
    ...
    {
        return isUpgradedWith(s, FocusUpgrades.diffusion) ? ItemFocusBasic.WandFocusAnimation.CHARGE :
    ...
    public ItemStack onFocusRightClick(ItemStack s, World w, EntityPlayer p, MovingObjectPosition mop)
    {
        ...
        else
        {
            if (!w.isRemote)
            {
                ...
                {
                    EntityDarkMatter proj = new EntityDarkMatter(w, p, 16F + wand.getFocusPotency(s),
    ...
    public void onUsingFocusTick(ItemStack s, EntityPlayer p, int i)
    {
        ...
        if (!p.worldObj.isRemote && p.ticksExisted % 5 == 0)
            p.worldObj.playSoundAtEntity(p, "thaumcraft:wind", 0.33F, 5.0F * (float) Math.random());

        if (!p.worldObj.isRemote && wand.consumeAllVis(s, p, getVisCost(s), true, false))
        {
            for (int a = 0; a < 2 + wand.getFocusPotency(s); a++)
            {
                EntityDiffusion proj = new EntityDiffusion(p.worldObj, p, isUpgradedWith(wand.getFocusItem(s),
                        FocusUpgradeType.enlarge) ? 15.0F : 8.0F, 12F + wand.getFocusPotency(s),
                        isUpgradedWith(wand.getFocusItem(s), FocusUpgrades.corrosive));
