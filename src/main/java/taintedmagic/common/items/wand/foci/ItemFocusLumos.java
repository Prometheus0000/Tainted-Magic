...
public class ItemFocusLumos extends ItemFocusBasic
{
    ...
    private static final AspectList costGlowpet =
            new AspectList().add(Aspect.ORDER, 500).add(Aspect.AIR, 500).add(Aspect.FIRE, 1000);
    private static final AspectList costMaxima =
            new AspectList().add(Aspect.ORDER, 1000).add(Aspect.FIRE, 1500).add(Aspect.AIR, 500);
    ...
    public void addInformation(ItemStack s, EntityPlayer p, List l, boolean b)
    {
        ...
        {
            l.add(" ");
            l.add(EnumChatFormatting.BLUE + "+" + new String(this.isUpgradedWith(s, FocusUpgradeType.enlarge) ?
                    Integer.toString(11 + this.getUpgradeLevel(s, FocusUpgradeType.enlarge)) : "11") + " " + StatCollector.translateToLocal("text.radius"));
        }
        if (this.isUpgradedWith(s, FocusUpgrades.glowpet))
        {
            l.add(" ");
            l.add(EnumChatFormatting.BLUE + new String(this.isUpgradedWith(s, FocusUpgradeType.extend) ?
                    Integer.toString(1 + this.getUpgradeLevel(s, FocusUpgradeType.extend)) : "1") + "x " + StatCollector.translateToLocal("text.burnTime"));
        }
    }

    public AspectList getVisCost(ItemStack s)
    {
        return isUpgradedWith(s, FocusUpgrades.maxima) ? this.costMaxima : isUpgradedWith(s, FocusUpgrades.glowpet) ?
                this.costGlowpet : this.cost;
    }
    ...
    {
        FXSparkle fx = new FXSparkle(w, x + w.rand.nextFloat(), y + w.rand.nextFloat(), z + w.rand.nextFloat(), 1.75F
                , 6, 3 + w.rand.nextInt(3));
        fx.setGravity(0.1F);
    ...
    void spawnMaximaParticles(World w, EntityPlayer p, float ex)
    {
        for (int i = 1; i < 100 + (25 * ex); i++)
        {
            ...
            FXWisp ef = new FXWisp(w, p.posX + xp + off, p.posY + 5.0D + yp + off, p.posZ + zp + off,
                    0.5F + (float) Math.random() * 0.25F, red, green, blue);
            ef.setGravity(0.0F);
            ef.shrink = true;
            ef.noClip = false;
    ...
    public FocusUpgradeType[] getPossibleUpgradesByRank(ItemStack s, int r)
    {
        switch (r)
        {
            ...
            case 2:
                if (isUpgradedWith(s, FocusUpgrades.glowpet))
                    return new FocusUpgradeType[]{FocusUpgradeType.frugal, FocusUpgradeType.extend};
                else if (isUpgradedWith(s, FocusUpgrades.maxima))
