...
public class ItemShadowmetalHoe extends ItemHoe implements IRepairable
{
    ...
    @Override
    public boolean onItemUse(ItemStack s, EntityPlayer p, World w, int x, int y, int z, int i, float a, float b,
                             float c)
    {
        ...
        if (!p.worldObj.isRemote)
        {
            ...
            if (p.worldObj.getBlock(x, y, z) == Blocks.farmland)
            {
                ...
                s.damageItem(1, p);
                w.playSoundEffect((double) ((float) x + 0.5F), (double) ((float) y + 0.5F),
                        (double) ((float) z + 0.5F), block.stepSound.getStepResourcePath(),
        ...
        if (p.worldObj.getBlock(x, y, z) == Blocks.dirt || p.worldObj.getBlock(x, y, z) == Blocks.grass || p.worldObj.getBlock(x, y, z) == Blocks.farmland)
            p.swingItem();
        return false;
    }
