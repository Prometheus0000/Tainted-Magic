...
public class ItemMagicFunguar extends ItemFood
{
    ...
    protected void onFoodEaten(ItemStack s, World w, EntityPlayer p)
    {
        ...
        if (!w.isRemote)
        {
            Thaumcraft.proxy.playerKnowledge.addAspectPool(p.getCommandSenderName(), aspects[i],
                    Short.valueOf((short) (1)));
            ResearchManager.scheduleSave(p);
            PacketHandler.INSTANCE.sendTo(new PacketAspectPool(aspects[i].getTag(), Short.valueOf((short) 1),
                    Short.valueOf(Thaumcraft.proxy.playerKnowledge.getAspectPoolFor(p.getCommandSenderName(), aspects[i]))), (EntityPlayerMP) p);
        }
    }
