...
public class EntityHomingShard extends EntityThrowable implements IEntityAdditionalSpawnData
{
    ...
    public EntityHomingShard(World w, EntityLivingBase p, EntityLivingBase t, int strength, boolean b)
    {
        ...
        Vec3 v = p.getLookVec();
        setLocationAndAngles(p.posX + v.xCoord / 2.0D, p.posY + p.getEyeHeight() + v.yCoord / 2.0D,
                p.posZ + v.zCoord / 2.0D, p.rotationYaw, p.rotationPitch);
        float f = 0.5F;
        ...
        this.motionX =
                (-MathHelper.sin(ry / 180.0F * (float) Math.PI) * MathHelper.cos(rp / 180.0F * (float) Math.PI) * f);
        this.motionZ =
                (MathHelper.cos(ry / 180.0F * (float) Math.PI) * MathHelper.cos(rp / 180.0F * (float) Math.PI) * f);
    ...
    public void onUpdate()
    {
        ...
        if (motion > 0.0D && this.worldObj.isRemote)
            Thaumcraft.proxy.sparkle((float) this.posX + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) * 0.25F, (float) this.posY + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) * 0.25F, (float) this.posZ + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) * 0.25F, 0.5F, 0, 0.0F);
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            {
                ArrayList<Entity> ents = EntityUtils.getEntitiesInRange(this.worldObj, this.posX, this.posY,
                        this.posZ, this, this.tClass, 16.0D);
