...
public class EntityTaintBubble extends EntityThrowable implements IEntityAdditionalSpawnData
{
    ...
    public void onUpdate()
    {
        ...
        ChunkCoordinates c = new ChunkCoordinates((int) this.posX + this.rand.nextInt(7) - this.rand.nextInt(7),
    ...
    protected void onImpact(MovingObjectPosition mop)
    {
        ...
        {
            List l = this.worldObj.getEntitiesWithinAABBExcludingEntity(getThrower(), this.boundingBox.expand(1.0D,
                    1.0D, 1.0D));
            ...
            for (int i = 0; i < l.size(); i++)
            {
                ...
                if (e instanceof EntityLivingBase)
                {
                    ...
                    {
                        ((EntityLivingBase) e).attackEntityFrom(new EntityDamageSourceIndirect("taint", this,
                                getThrower()).setMagicDamage(), this.dmg);
                        if (this.corrosive)
                        {
                            try
                            {
